...
class UploadFile
{
	...
	function __construct($type,$id='')
	{
        ...
        $this->path = UPLOAD_DIR.$type.'/'. $this->id;
        $this->imgUrl = UPLOAD_URL.$type.'/'. $this->id;
        $this->type = $type;
     ...
     * @str $path   保存路径(物理)
     * */
    public function plainFileUpload($data,$path=''){
        
        $fileFunction = $this->type.'FileUpload';
        $key=$this->type.'Upload';
        $_FILES[$key]['name'] = $data['name'];
        ...
        if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64, $result)) {
            /*文件新名称*/
            $TempFileName = time(). rand(1000,9999) . "." . $result['2'];
            /*原图路径文件*/
    ...
    public function getUploadFiles($fileName,$prefix='',$id=''){
        ...
        if(!empty($id)){
            ...
            $this->path = UPLOAD_DIR.$this->type.'/'. $id;
            $this->imgUrl = UPLOAD_URL.$this->type.'/'. $id;
        }
